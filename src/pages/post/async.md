---
  ...
  medium: "https://medium.com/@jossdz/camino-para-conocer-la-programaci%C3%B3n-as%C3%ADncrona-en-javascript-1622c8af75f7"
  tags: ["js","nodejs"]
...
```javascript
function comprarTaquito(taquitosActuales){
    let promise, taquitos = taquitosActuales + 1
    return promise = new Promise(function(resolve){
        resolve(taquitos)
    })
}
function comprarTresTaquitos(taquitosActuales){
    let promise, taquitos = taquitosActuales + 3
    return promise = new Promise(function(resolve){
        resolve(taquitos)
    })
}
function comprarCincoTaquitos(taquitosActuales){
    let promise, taquitos = taquitosActuales + 5
    return promise = new Promise(function(resolve){
        resolve(taquitos)
    })
}
...
comprarTaquito(tacos)
    .then(function(result){
        console.log('taquitos:', result)
        comprarTresTaquitos(result)
            .then(function(result){
                console.log('taquitos:', result)
                comprarCincoTaquitos(result)
                    .then(function(result){
                    console.log('taquitos:', result)
                    })
            })
...
El ejercicio luce igual o más enredoso y aun que no lo parezca esto es un gran paso adelante en la programación asíncrona con javascript. Ahora tenemos una palabra clave [**then**](https://developer.mozilla.org/es/docs/Web/JavaScript/Referencia/Objetos_globales/Promise/then) que recibe un callback(😩) el cual recibe el resultado de la promesa como primer argumento, esto aclara el código y no tenemos lógica confusa en tantos callbacks.

En un caso más real es cuando las promesas tienen un impacto y aclaran el código, añadiendo incluso una mejor forma de manejar los errores con el método [**catch**](https://developer.mozilla.org/es/docs/Web/JavaScript/Referencia/Objetos_globales/Promise/catch) de las promesas.
...
Echa un vistazo a la manera definitiva que ofrece node desde la versión 8 y [babel](https://babeljs.io/).
...
async function taquitos(){
    const primerCompra = await comprarTaquito(tacos)
    console.log('taquitos: ' + primerCompra)
    const SegundaCompra = await comprarTresTaquitos(primerCompra)
    console.log('taquitos: ' + SegundaCompra)
    const TercerCompra = await comprarCincoTaquitos(SegundaCompra)
    console.log('taquitos: ' + TercerCompra)
}
taquitos()
```
...
![async](https://cdn-images-1.medium.com/max/1600/1*baE6-Uk82W9ZBu18HFrEzg.png)
...
```javascript
const comprarTaquito = taquitosActuales => {
    let promise, taquitos = taquitosActuales + 1
    return promise = new Promise(resolve => resolve(taquitos))
}
const comprarTresTaquitos = taquitosActuales => {
    let promise, taquitos = taquitosActuales + 3
    return promise = new Promise(resolve => resolve(taquitos))
}
const comprarCincoTaquitos = taquitosActuales => {
    let promise, taquitos = taquitosActuales + 5
    return promise = new Promise(resolve => resolve(taquitos))
}
// no tengo taquitos :(
